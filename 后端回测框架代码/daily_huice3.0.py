...
class Result(Base):
    __tablename__ = 'first_page_table_daily_factor'
...
def store_results_to_database(engine, factor_column, table_name, last_longshort_return, annualized_returns_season, annualized_volatility_season, sharpe_ratio_season, ic_mean_season, IR_season):
    ...
    # 创建一个结果对象并添加到数据库中
    result = Result(table_name=table_name, english_factor=factor_column, cumulative_returns_with_fee=cumulative_returns_with_fee_season, annualized_returns=annualized_returns_season, annualized_volatility=annualized_volatility_season, sharpe_ratio=sharpe_ratio_season, ic_mean=ic_mean_season,
                    IR=IR_season)
    session.add(result)
...
if __name__ == "__main__":
    ...
    for factor_column in factors:
        ...
        #日调仓时点收益率
        daily_returns = calculate_returns(close, window=1)  # 数据是一季度级别数据
        ...
        store_results_to_database(engine, factor_column, table_name, last_longshort_return, annualized_returns, annualized_volatility_season, sharpe_ratio_season, ic_mean_season, IR_season)
