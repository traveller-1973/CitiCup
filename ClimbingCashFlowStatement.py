...
if __name__ == '__main__':
    ...
    first_column_data = []
    for row in range(3495, sheet.nrows):
  ...
  #  result = [item.split('.')[0]for item in first_column_data]
    ...
    for stock_code in first_column_data:
        print(stock_code)
        # pro = ts.pro_api()
        df = pro.cashflow(ts_code=stock_code, start_date='20130101', end_date='20240304')
                        # ,
                        # fields='ts_code,ann_date,f_ann_date,end_date,report_type,comp_type,basic_eps,diluted_eps')
        print(df)
        df.to_sql(name='Cash_Flow_Statement', con=engine, if_exists='append', index=False)
        time.sleep(0.12)
